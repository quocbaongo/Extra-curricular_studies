...
```
    ...
    <ipython-input-12-55ff94ad14fb> in <module>
          2 triads_to_try = [('italy', 'italian', 'spain'), ('india', 'delhi', 'japan'), ('man', 'woman', 'boy'), ('small', 'smaller', 'large')]
          3 for triad in triads_to_try:
    ----> 4     print ('{} -> {} :: {} -> {}'.format( *triad, complete_analogy(*triad, word_to_vec_map)))
          5 
          6 # END SKIP FOR GRADING


    <ipython-input-9-3282888e6cd7> in complete_analogy(word_a, word_b, word_c, word_to_vec_map)
         37         ### START CODE HERE ###
         38         # Compute cosine similarity between the vector (e_b - e_a) and the vector ((w's vector representation) - e_c)  (≈1 line)
    ---> 39         cosine_sim = cosine_similarity(e_b - e_a, word_to_vec_map[w] - e_c)
         40 
         41         # If the cosine_sim is more than the max_cosine_sim seen so far,


    <ipython-input-4-c1692bc57c68> in cosine_similarity(u, v)
         15 
         16     # Special case. Consider the case u = [0, 0], v=[0, 0]
    ---> 17     if np.all(u == v):
         18         return 1
         19 
...
```
    ...
    100%|██████████| 400000/400000 [00:04<00:00, 91738.27it/s] 
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    corrected_e_w1B = np.sqrt(1 - np.sum(mu_orth ** 2)) * ((e_w1B - mu_B) / np.sqrt(np.sum((e_w1B - mu_B)**2)))
    corrected_e_w2B = np.sqrt(1 - np.sum(mu_orth ** 2)) * ((e_w2B - mu_B) / np.sqrt(np.sum((e_w2B - mu_B)**2)))
...
```
    ...
    cosine_similarity(e1, gender) =  -0.23871136142883792
