...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, training, mask):
        ...
        #query = tf.keras.Input(shape=(x.shape[0], x.shape[1], x.shape[2]))
        #key = tf.keras.Input(shape=(x.shape[0], mask.shape[-1], x.shape[2]))
        #value = tf.keras.Input(shape=(x.shape[0], mask.shape[-1], x.shape[2]))
        #self_mha_output = self.mha(query=query, key=key, value=value, return_attention_scores=False, training=training)  # Self attention (batch_size, input_seq_len, embedding_dim)
        self_mha_output = self.mha(x, x, x, mask)
        
        # skip connection
        # apply layer normalization on sum of the input and the attention output to get the  
        # output of the multi-head attention layer (~1 line)
        skip_x_attention = self.layernorm1(tf.add(x, self_mha_output))  # (batch_size, input_seq_len, embedding_dim)
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training, look_ahead_mask, padding_mask):
        ...
        mult_attn_out2 = self.layernorm1(tf.add(Q1, mult_attn_out2))  # (batch_size, target_seq_len, embedding_dim)
        ...
        out3 = self.layernorm3(tf.add(mult_attn_out2, ffn_output))  # (batch_size, target_seq_len, embedding_dim)
        # END CODE HERE
