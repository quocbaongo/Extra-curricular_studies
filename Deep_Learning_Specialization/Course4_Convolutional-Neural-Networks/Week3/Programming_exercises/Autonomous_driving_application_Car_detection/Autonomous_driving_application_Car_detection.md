...
# Inputs
# boxes: shape (19, 19, 5, 4) - containing the midpoint 
#and dimensions (𝑏𝑥,𝑏𝑦,𝑏ℎ,𝑏𝑤) for each of the 5 boxes in each cell.

# box_confidence: shape (19,19,5,1) - containing 𝑝𝑐 (confidence probability that there's some object) 
#for each of the 5 boxes predicted in each of the 19x19 cells.

# box_class_probs: shape (19,19,5,80) - containing the "class probabilities" (𝑐1,𝑐2,...𝑐80) 
...
# Step 2: Find the box_classes using the max box_scores, keep track of the corresponding score
box_classes = tf.math.argmax(box_scores, axis=-1) # shape of box_classes (19, 19, 5), 
                                                #idx of max value in each of 5 anchor boxes within a grid cell

box_class_scores = tf.math.reduce_max(box_scores, axis=-1) # shape of box_class_scores (19, 19, 5), 
                                                #max value in each of 5 anchor boxes within a grid cell

# Step 3: The mask should be True for the boxes you want to keep (score larger than threshold) 
filtering_mask = box_class_scores >= threshold # shape of filtering_mask (19, 19, 5)

# Step 4: Apply the mask to box_class_scores, boxes and box_classes
scores = tf.boolean_mask(box_class_scores, filtering_mask) # shape of scores (1786,) reduced from original of (19, 19, 5)
                                                        # scores of selected anchor boxes throughout 19 x 19 grid cells
    
    
boxes = tf.boolean_mask(boxes, filtering_mask) # shape of scores (1786, 4)
                                            #dimensions (𝑏𝑥,𝑏𝑦,𝑏ℎ,𝑏𝑤) of selected anchor boxes
                                            #throughout 19 x 19 grid cells

classes = tf.boolean_mask(box_classes, filtering_mask) # shape of classes (1786,)
                                                    #predicted classes of selected anchor boxes
                                                    #throughout 19 x 19 grid cells
```
...
```python
# Test
# scores, boxes and classes are the output of yolo_filter_boxes() function
# Inputs
...
# Execution yolo_non_max_suppression
boxes = tf.cast(boxes, dtype=tf.float32)
scores = tf.cast(scores, dtype=tf.float32)
...
for label in classes_labels:
    filtering_mask = classes == label # Output tf.Tensor([ True False], shape=(2,), dtype=bool) and
                                    # tf.Tensor([False  True], shape=(2,), dtype=bool) in the next iteration

    # Get boxes for this class
    # Use tf.boolean_mask() with 'boxes' and `filtering_mask`
    boxes_label = tf.boolean_mask(boxes, filtering_mask) # Output of tf.Tensor([[0.45 0.2  1.01 2.6 ]], shape=(1, 4), dtype=float32) for the first iteration and
                                                        # tf.Tensor([[0.42 0.15 1.7  1.01]], shape=(1, 4), dtype=float32) for the next iteration
        
    
    # Get scores for this class
    # Use tf.boolean_mask() with 'scores' and `filtering_mask`
    scores_label = tf.boolean_mask(scores, filtering_mask) # Output of tf.Tensor([0.855], shape=(1,), dtype=float32) for the first iteration and
                                                        # tf.Tensor([0.828], shape=(1,), dtype=float32) for the next iteration
    ...
    if tf.shape(scores_label)[0] > 0:  # Check if there are any boxes to process
        ...
        nms_indices_label = tf.image.non_max_suppression(
                                                        ...
                                                        max_boxes,
                                                        iou_threshold=iou_threshold)
        # Output of nms_indices_label for the first iteration: tf.Tensor([0], shape=(1,), dtype=int32)
        # Output of nms_indices_label for the second iteration: tf.Tensor([0], shape=(1,), dtype=int32)
        
        
        # Get original indices of the selected boxes
        selected_indices = tf.squeeze(tf.where(filtering_mask), axis=1)
        
        # Output of selected_indices for the first iteration: tf.Tensor([0], shape=(1,), dtype=int64)
        # Output of selected_indices for the second iteration: tf.Tensor([1], shape=(1,), dtype=int64)
           
        # Append the resulting boxes into the partial result
        # Use tf.gather() with 'selected_indices' and `nms_indices_label`
        nms_indices.append(tf.gather(selected_indices, nms_indices_label))
        
        # Output of nms_indices for the first iteration: [<tf.Tensor: shape=(1,), dtype=int64, numpy=array([0])>]
        # Output of nms_indices for the second iteration: [<tf.Tensor: shape=(1,), dtype=int64, numpy=array([0])>, <tf.Tensor: shape=(1,), dtype=int64, numpy=array([1])>]

        
# Flatten the list of indices and concatenate
# Use tf.concat() with 'nms_indices' and `axis=0`
nms_indices = tf.concat(nms_indices, axis=0) # nms_indices: tf.Tensor([0 1], shape=(2,), dtype=int64)
...
# Final output: each object in the image should be described by only 1 anchor box.
